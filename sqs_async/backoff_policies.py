...
class ExponentialBackoff(BackoffPolicy):
    ...
    def __init__(
        self, base=2, min_visibility_timeout=0, max_visbility_timeout=30 * 60
    ):
    ...
    def get_visibility_timeout(self, message):
        ...
        visibility_timeout = max(
            self.min_visibility_timeout, visibility_timeout
        )
        visibility_timeout = min(
            self.max_visibility_timeout, visibility_timeout
        )
